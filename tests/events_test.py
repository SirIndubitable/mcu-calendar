...
def test_shows_yaml(show_path):
    for show in Show.from_yaml(os.path.join('data', 'shows', show_path)):
...
def test_shows_equals():
    show1 = Show({'title': "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 7, 'num': 1}], 'description': "Lots of stuff"}, season_num=1)
    show2 = Show({'title': "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 7, 'num': 1}], 'description': "Lots of stuff"}, season_num=1)
    assert show1 == show1
    assert show1 == show2
...
@pytest.mark.parametrize("show_dict", [
    { "title": "MY TITLE 2", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 3, 'num': 1}], "description": "Sometimes things happen" },
    { "title": "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 12, 25), 'weeks':3, 'num': 1}], "description": "Sometimes things happen" },
    { "title": "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 9, 'num': 1}], "description": "Sometimes things happen" },
    { "title": "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 9, 'num': 1}], "description": "Nothing ever happens" },
])
def test_shows_not_equals(show_dict):
    show1 = Show({'title': "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 3, 'num': 1}], 'description':"Sometimes things happen"}, season_num=1)
    show2 = Show(show_dict, season_num=1)
    assert not show1 == show2
    assert show1 != show2


def test_show_event_equals():
    show = Show({'title': "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 20, 'num': 1}], 'description':"Sometimes things happen"}, season_num=1)
    event = {
        "start": { "date": "2019-04-20" },
        "end": { "date": "2019-04-21" },
        "summary": "MY TITLE",
...
@pytest.mark.parametrize("event_dict", [
    { "start": { "date": "2019-10-20" }, "end": { "date": "2019-04-21" }, "summary": "MY TITLE", "recurrence": "RRULE:FREQ=WEEKLY;WKST=SU;COUNT=6;BYDAY=SA", "description": "Sometimes things happen" },
    { "start": { "date": "2019-04-20" }, "end": { "date": "2019-03-14" }, "summary": "MY TITLE", "recurrence": "RRULE:FREQ=WEEKLY;WKST=SU;COUNT=6;BYDAY=SA", "description": "Sometimes things happen" },
    { "start": { "date": "2019-04-20" }, "end": { "date": "2019-04-21" }, "summary": "YOUR TITLE", "recurrence": "RRULE:FREQ=WEEKLY;WKST=SU;COUNT=6;BYDAY=SA", "description": "Sometimes things happen" },
    { "start": { "date": "2019-04-20" }, "end": { "date": "2019-04-21" }, "summary": "MY TITLE", "recurrence": "RRULE:FREQ=WEEKLY;WKST=SU;COUNT=10;BYDAY=SA", "description": "Sometimes things happen" },
    { "start": { "date": "2019-04-20" }, "end": { "date": "2019-04-21" }, "summary": "MY TITLE", "recurrence": "RRULE:FREQ=WEEKLY;WKST=SU;COUNT=6;BYDAY=SA", "description": "Nothing ever happens" },
])
def test_show_event_not_equals(event_dict):
    show = Show({'title': "MY TITLE", 'seasons': [{'start_date': datetime.date(2019, 4, 20), 'weeks': 6, 'num': 1}], 'description':"Sometimes things happen"}, season_num=1)
